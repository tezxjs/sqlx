...
function condition(filters, joinBy = "AND") {
    ...
    for (let column in filters) {
        ...
        if (column === "$and") {
            ...
            if (Array.isArray(value)) {
                part = `${col} IN ${(0, sanitize_js_1.sanitize)(value)}`;
            }
            else {
                if (Array.isArray(value?.notIn) && value?.notIn?.length) {
                    part = `${col} NOT IN ${(0, sanitize_js_1.sanitize)(value?.notIn)}`;
                }
                if (Array.isArray(value?.in) && value?.in?.length) {
                    part = `${col} IN ${(0, sanitize_js_1.sanitize)(value?.in)}`;
                }
                if (Array.isArray(value?.between) && value?.between?.length == 2) {
                    part = `${col} BETWEEN ${(0, sanitize_js_1.sanitize)(value.between[0])} AND ${(0, sanitize_js_1.sanitize)(value.between[1])}`;
                }
                if (Array.isArray(value?.notBetween) && value?.notBetween?.length == 2) {
                    part = `${col} NOT BETWEEN ${(0, sanitize_js_1.sanitize)(value.notBetween[0])} AND ${(0, sanitize_js_1.sanitize)(value.notBetween[1])}`;
                }
                if (Array.isArray(value?.inRange) && value?.inRange?.length == 2) {
                    part = `${col} BETWEEN ${(0, sanitize_js_1.sanitize)(value.inRange[0])} AND ${(0, sanitize_js_1.sanitize)(value.inRange[1])}`;
                }
                ...
                if (value?.eq) {
                    part = `${col} = ${(0, sanitize_js_1.sanitize)(value.eq)}`;
                }
                if (value?.gt) {
                    part = `${col} > ${(0, sanitize_js_1.sanitize)(value.gt)}`;
                }
                if (value?.lt) {
                    part = `${col} < ${(0, sanitize_js_1.sanitize)(value.lt)}`;
                }
                if (value?.gte) {
                    part = `${col} >= ${(0, sanitize_js_1.sanitize)(value.gte)}`;
                }
                if (value?.lte) {
                    part = `${col} <= ${(0, sanitize_js_1.sanitize)(value.lte)}`;
                }
                if (value?.neq) {
                    part = `${col} != ${(0, sanitize_js_1.sanitize)(value.neq)}`;
                }
            ...
            if (conditions)
                conditions += ` ${joinBy} `;
            conditions += `${column} = ${(0, sanitize_js_1.sanitize)(value)}`;
        }
    }
    return conditions;
}
