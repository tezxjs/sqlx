...
export interface FindOneParamsType<Tables extends string[]> {
   ...
   */
  columns?: { [P in Tables[number]]?: string[] }
  | { extra?: string | string[] }
  | string[]
  | string;
 ...
 */
export function find<Tables extends string[]>(
  table: string,
  ...
  if (aggregates && aggregates.length) {
    for (let i = 0; i < aggregates.length; i++) {
      ...
      for (const func in agg) {
        ...
        if (!Object.prototype.hasOwnProperty.call(agg, func)) continue;
        const col = (agg as any)[func];
        if (!col) continue;
        if (select) select += ", ";

        if (!alias) {
          alias = aggregates_alias[func as keyof typeof aggregates_alias] || func;
        }
